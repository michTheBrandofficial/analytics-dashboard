  ...
  return (
    <Section.Bordered className='font-normal px-3 pt-2 pb-2 column justify-between text-text_11 '>
      <HStack className='w-full h-fit items-center justify-between'>
        ...
        <div className='fit'>
          <Show switch={increase} when={() => increase.value} fallback={<GraphDown />} >
            <GraphUp />
          </Show>
        </div>
      </HStack>
      ...
      <p className="text-lg text-text_1 " >{props?.name === 'income' ? formatNum(props?.total) : props?.total}</p>

      <HStack className="w-full h-fit items-center justify-between text-sm">
        <div className={concat`fit px-2 py-1 rounded-full ${growth} flex items-center gap-2`}>
        <Show switch={increase} when={() => increase.value} fallback={<DownArrow />} >
            <UpArrow />
          </Show>
